...
function product() {
  ...
  var product = JSON.parse(productForm.querySelector('#js-product').value);
  var optionRadios = Array.prototype.slice.call(productForm.querySelectorAll('[type="radio"]'));
  var priceEl = productForm.querySelector('.js-price');
  var productSelect = productForm.querySelector('#productSelect');
  var productRadios = productForm.querySelector('.product-radios');
  var moneyFormat = productForm.querySelector('#js-money-format').value;
  var submitBtn = productForm.querySelector('#AddToCart');
  var qtySelect = productForm.querySelector('#Quantity');
  var msgEl = productForm.querySelector('#message');
  var cartCount = Array.prototype.slice.call(document.querySelectorAll('.cart-link__count')); // Vars

  var variants = product.variants;
  var productVariants = variants.map(function (variant) {
    variant.inventory_quantity = parseInt(document.querySelector("[name=\"".concat(variant.id, "\"]")).value);
    return variant;
  });

  var lowStockThreshold = parseInt(document.getElementById('low-stock-threshold').value);
  var maxQty = parseInt(document.getElementById('max-qty').value);
  var soldOutMsg = document.getElementById('sold-out-msg').value;
  var lowStockMsg = document.getElementById('low-stock-msg').value;
  var selling_plan = product.selling_plan_groups[0].selling_plans[0].id;
  ...
  optionRadios.forEach(function (radio) {
    radio.addEventListener('change', handleUpdate, false);
  });
  productForm.addEventListener('submit', function (e) {
    ...
    var props = {
      "shipping_interval_frequency": document.querySelector('#rc_shipping_interval_frequency') ? document.querySelector('#rc_shipping_interval_frequency').value : null,
      "shipping_interval_unit_type": document.querySelector('#rc_shipping_interval_unit_type') ? document.querySelector('#rc_shipping_interval_unit_type').value : null
    };
    var addData = {
      "id": getVariant(getOptions()).id,
      "quantity": parseInt(qtySelect.value),
      "selling_plan": selling_plan
    };

    if (props["shipping_interval_frequency"]) {
      addData.properties = props;
    }
  ...
  function getVariant(options) {
    ...
    return {
      ...
      stock: variant ? variant.inventory_quantity : null,
      inventory: variant ? variant.inventory_management : null
    };
  } // Need to run this on inint


  function handleUpdate() {
    var variant = getVariant(getOptions());
    console.log(variant);
    ...
    if (variant.stock === 0 && variant.inventory) {
      return handleSoldOut();
    }

    if (variant.stock <= lowStockThreshold && variant.inventory || variant.stock < maxQty && variant.inventory) {
      return handleLowStock(variant);
    }

    return handleOk(variant);
  }

  function handleSoldOut() {
    submitBtn.setAttribute('disabled', 'disabled');
    updateMsg(soldOutMsg);
    return;
  }

  function handleUnavailable() {
    submitBtn.setAttribute('disabled', 'disabled');
    updateMsg('Sorry. Not available in your chosen combination');
    return;
  }

  function handleLowStock(variant) {
    var msg = lowStockMsg.replace('####', variant.stock); // set max range on qty select

    if (variant.stock < maxQty) {
      fillQtySelect(variant.stock);
    } // show message
  ...
  function handleOk(variant) {
    submitBtn.removeAttribute('disabled');
    setPrice(variant);
    fillQtySelect(maxQty);
    hideMsg();
  }

  function setPrice(variant) {
    priceEl.innerHTML = Shopify.formatMoney(variant.price, moneyFormat);
  }

  function fillQtySelect(num) {
    var arr = _toConsumableArray(Array(num).keys());

    var opts = arr.map(function (el, i) {
      return "<option value=\"".concat(i + 1, "\">").concat(i + 1, "</option>");
    });
    qtySelect.innerHTML = opts.join('');
    return;
  }
  ...
  function addToCart(items) {
    fetch('/cart/add.js', {
      ...
      /* we have JSON */
      // Shopify error which is not returned as an error 😫
      if (json.status === 422) {
        console.error(json);
        updateMsg("Sorry. Something went wrong. ".concat(json.description, " Please try again. If the problem persists, please <a href=\"/pages/contact\">Contact us</a>"));
      ...
      _micromodal.default.show('add-to-cart-modal');
    }).catch(function (err) {
      /* uh oh, we have a REAL error. */
      console.error('add to cart err', err);
      updateMsg("Sorry. Something went wrong. Please try again. If the problem persists, please <a href=\"/pages/contact\">Contact us</a>");
      submitBtn.classList.remove('is-loading');
      submitBtn.innerHTML = 'Add to basket';
    });
  }
  ...
  function updateCartCount() {
    fetch('/cart.json').then(function (response) {
      ...
      /* we have JSON */
      // Update the desktop and mobile cart counts
  ...
  function init() {
    ...
    productRadios.classList.remove('hidden');
    handleUpdate();
  }

  init();
}
},{"micromodal":"../node_modules/micromodal/dist/micromodal.es.js"}],"../node_modules/@shopify/theme-addresses/loader.js":[function(require,module,exports) {
"use strict";
...
},{}],"../node_modules/@shopify/theme-addresses/helpers.js":[function(require,module,exports) {
"use strict";
...
},{}],"../node_modules/@shopify/theme-addresses/addressForm.js":[function(require,module,exports) {
"use strict";
...
},{"./loader":"../node_modules/@shopify/theme-addresses/loader.js","./helpers":"../node_modules/@shopify/theme-addresses/helpers.js"}],"../node_modules/@shopify/theme-addresses/theme-addresses.js":[function(require,module,exports) {
"use strict";
...
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
},{"./addressForm":"../node_modules/@shopify/theme-addresses/addressForm.js"}],"../node_modules/tiny-slider/src/helpers/Object.keys.js":[function(require,module,exports) {
// Object.keys
if (!Object.keys) {
...
},{}],"../node_modules/bluegg-tabs/dist/index.js":[function(require,module,exports) {
'use strict';
...
var alltabs = document.querySelectorAll('.js-tabs'); // Init the tabs
...
navToggle.addEventListener('click', handleScrollPos, false);
var siteNav = new Nav({
  nav: '.site-nav',
  trigger: '#nav-toggle',
  openClass: 'is-open',
  bodyClass: 'js-mobile-nav-open'
}); // Move site back to correct scroll pos when nav opens

var addressForm = document.querySelector('[data-address=root]');

if (addressForm) {
  // Fill in country and provinces
  (0, _themeAddresses.AddressForm)(addressForm, 'en');
}

if (document.getElementById('AddToCartForm') != null) {
  (0, _product.product)();
}

document.body.classList.remove('is-tab-user');

function handleFirstTab(e) {
  if (e.keyCode === 9) {
    // the "I am a keyboard user" key
    document.body.classList.add('is-tab-user');
    window.removeEventListener('keydown', handleFirstTab);
  }
}

window.addEventListener('keydown', handleFirstTab);
},{"./product.js":"product.js","@shopify/theme-addresses":"../node_modules/@shopify/theme-addresses/theme-addresses.js","tiny-slider/src/tiny-slider":"../node_modules/tiny-slider/src/tiny-slider.js","bluegg-nav":"../node_modules/bluegg-nav/dist/index.js","bluegg-tabs":"../node_modules/bluegg-tabs/dist/index.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
